...
async function main_enrollUser(newUser) {
    try {
        ...
        let test = fs.readFileSync('../gateway/connection-org1.yaml', {encoding: 'utf8', flag: 'r'})
        let connectionProfile = yaml.safeLoad(test);
      ...
      //  console.log("User: ");
        ...
        fs.mkdir("../identity/user/"+newUser+"/wallet", {
            recursive: true
        }, function (err) {
            if (err) {
                console.log(err)
        ...
        const walletPath = path.join(process.cwd(), '../identity/user/'+newUser+'/wallet');
...
module.exports={main_enrollUser};